...
def gather_project_context(repo_root: Path) -> str:
    ...
    if not context:
        print("❌ Error: No relevant Rust files found or project context is empty.")
...
def call_ai(prompt: str, config: dict, repo_root: Path) -> str:
    ...
    system_prompt = (
        "You are a chess engine expert analyzing what features are needed for a world-class engine. "
    ...
    return run_agent(system_prompt, prompt, config, repo_root, "features_analysis", "features")
