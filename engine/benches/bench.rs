...
const STOCKFISH_BENCH_FENS: [&str; 50] = [
...
fn main() {
    let depth = std::env::var("BENCH_DEPTH")
        .ok()
        .and_then(|s| s.parse::<usize>().ok())
        .unwrap_or(BENCH_DEPTH);
    let bench_limit = std::env::var("BENCH_LIMIT")
        .ok()
        .and_then(|s| s.parse::<usize>().ok())
        .unwrap_or(STOCKFISH_BENCH_FENS.len())
        .min(STOCKFISH_BENCH_FENS.len());

    let mut engine = Engine::new(65_536, SimpleMoveGen, MaterialEvaluator);

    println!("Cody bench (Stockfish-style)");
    println!("Depth: {}", depth);
    println!("Positions: {}", bench_limit);
    println!();

    let global_start = Instant::now();
    let mut total_nodes: u64 = 0;

    for (i, fen) in STOCKFISH_BENCH_FENS.iter().take(bench_limit).enumerate() {
        println!("Position: {}/{} ({})", i + 1, bench_limit, fen);
        let pos = Position::from_fen(fen);

        NODE_COUNT.store(0, Ordering::Relaxed);
        let start = Instant::now();
        let (_best_move, score) = engine.search(&pos, depth, None, None);
        let elapsed = start.elapsed();
        let nodes = NODE_COUNT.load(Ordering::Relaxed);
        total_nodes = total_nodes.saturating_add(nodes);

        println!(
            "bestscore cp {} nodes {} time {}",
            score,
            nodes,
            elapsed.as_millis()
        );
